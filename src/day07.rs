...
pub fn day07(input: String) {
    ...
    let input : Vec<Vec<_>> = input.lines().map(|x|x.split("contain").collect()).collect();

    let remove_bag_suffix = Regex::new(r" (bag|bags)\.*$").unwrap();

    for line in input {
        let name = remove_bag_suffix.replace(line[0].trim(), "");
        let contains : Vec<_> = line[1].split(",").map(|x|remove_bag_suffix.replace(x.trim(),"")).collect();
        let contains = contains.iter().map(|x| {
            x.splitn(2, " ").collect::<Vec<_>>()
        }).map(|x| {
            if x[0] == "no" {
                None
            } else {
        ...
        .map(|(num,x)| {
            let bag = if bags.contains_key(&String::from(x)) {
                ...
                bags.insert(String::from(x), Rc::new(RefCell::new(Bag{name: String::from(x), contains: vec![]})));
                bags.get(&String::from(x)).unwrap().clone()
            };
            (num, bag)
        }).collect::<Vec<_>>();
        ...
        let bag = if bags.contains_key(&String::from(name.clone())) {
            ...
            bags.insert(String::from(name.clone()), Rc::new(RefCell::new(Bag{name: String::from(name.clone()), contains: vec![]})));
            bags.get(&String::from(name)).unwrap().clone()
        };
    ...
    let res = Rc::new(RefCell::new(bags.get("shiny gold").unwrap().borrow().clone()));
